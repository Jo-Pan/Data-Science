...
class SupervisedGraphsage(models.SampleAndAggregate):
    ...
    def __init__(self, num_classes,
        ...
        if features is None: 
            ...
            if sys.getsizeof(features) >= 2147483648: # if features >2GB
                self.features_init = tf.placeholder(tf.float32, shape=features.shape)
                self.features = tf.Variable(self.features_init, trainable=False)
            else:
